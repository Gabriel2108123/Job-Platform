import { ROUTES } from '../routes';

export const candidateNav = [
    { label: 'Home', icon: 'grid', href: ROUTES.CANDIDATE.HOME },
    { label: 'Browse Jobs', icon: 'briefcase', href: ROUTES.CANDIDATE.JOBS },
    { label: 'Applications', icon: 'file-text', href: ROUTES.CANDIDATE.APPLICATIONS },
    { label: 'CV & Documents', icon: 'file-text', href: ROUTES.CANDIDATE.DOCUMENTS },
    { label: 'Messages', icon: 'message-square', href: ROUTES.CANDIDATE.MESSAGES },
    { label: 'Network', icon: 'network', href: ROUTES.CANDIDATE.NETWORK },
];
