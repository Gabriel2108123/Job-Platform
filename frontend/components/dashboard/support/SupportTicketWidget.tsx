    ...
    return (
        <Card className="rounded-[2.5rem] border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden bg-white dark:bg-slate-900 border-none shadow-slate-200/50">
            <CardBody className="p-8">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-8">Recent Tickets</h3>
                <div className="space-y-6">
                    {tickets.map(ticket => (
                        <div key={ticket.id} className="flex items-center justify-between group cursor-pointer">
                            ...
                            <div className={`px-2 py-0.5 rounded-md text-xs font-black uppercase tracking-widest ${ticket.status === 'Priority' ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'
                                }`}>
                                {ticket.status}
                            </div>
                        </div>
